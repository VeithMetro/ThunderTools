...
namespace ProxyStubs {
    ...
    static ProxyStub::MethodHandler ExchangeConnectionPropertiesStubMethods[] = {
        ...
        [](Core::ProxyType<Core::IPCChannel>& channel, Core::ProxyType<RPC::InvokeMessage>& message) {
            ...
            RPC::Data::Frame::Reader reader(message->Parameters().Reader());
            Core::instance_id _parameter_a965c526InstanceId__ = reader.Number<Core::instance_id>();

            Exchange::IConnectionProperties::INotification* _parameter_a965c526{};
            ProxyStub::UnknownProxy* _parameter_a965c526Proxy__ = nullptr;
            if (_parameter_a965c526InstanceId__ != 0) {
                _parameter_a965c526Proxy__ = RPC::Administrator::Instance().ProxyInstance(channel, _parameter_a965c526InstanceId__, false, _parameter_a965c526);
                ASSERT((_parameter_a965c526 != nullptr) && (_parameter_a965c526Proxy__ != nullptr));
            }

            uint32_t result = implementation->Register(_parameter_a965c526);

            RPC::Data::Frame::Writer writer(message->Response().Writer());
            writer.Number<uint32_t>(result);

            if (_parameter_a965c526Proxy__ != nullptr) {
                RPC::Administrator::Instance().Release(_parameter_a965c526Proxy__, message->Response());
            }
        },
        ...
        [](Core::ProxyType<Core::IPCChannel>& channel, Core::ProxyType<RPC::InvokeMessage>& message) {
            ...
            RPC::Data::Frame::Reader reader(message->Parameters().Reader());
            Core::instance_id _parameter_aeb92231InstanceId__ = reader.Number<Core::instance_id>();

            Exchange::IConnectionProperties::INotification* _parameter_aeb92231{};
            ProxyStub::UnknownProxy* _parameter_aeb92231Proxy__ = nullptr;
            if (_parameter_aeb92231InstanceId__ != 0) {
                _parameter_aeb92231Proxy__ = RPC::Administrator::Instance().ProxyInstance(channel, _parameter_aeb92231InstanceId__, false, _parameter_aeb92231);
                ASSERT((_parameter_aeb92231 != nullptr) && (_parameter_aeb92231Proxy__ != nullptr));
            }

            uint32_t result = implementation->Unregister(_parameter_aeb92231);

            RPC::Data::Frame::Writer writer(message->Response().Writer());
            writer.Number<uint32_t>(result);

            if (_parameter_aeb92231Proxy__ != nullptr) {
                RPC::Administrator::Instance().Release(_parameter_aeb92231Proxy__, message->Response());
            }
        },
    ...
    public:
        ...
        uint32_t Register(Exchange::IConnectionProperties::INotification* _parameter_a965c526) override
        {
            IPCMessage message(static_cast<const ProxyStub::UnknownProxy&>(*this).Message(0));

            RPC::Data::Frame::Writer writer(message->Parameters().Writer());
            writer.Number<Core::instance_id>(RPC::instance_cast(_parameter_a965c526));
        ...
        uint32_t Unregister(Exchange::IConnectionProperties::INotification* _parameter_aeb92231) override
        {
            IPCMessage message(static_cast<const ProxyStub::UnknownProxy&>(*this).Message(1));

            RPC::Data::Frame::Writer writer(message->Parameters().Writer());
            writer.Number<Core::instance_id>(RPC::instance_cast(_parameter_aeb92231));
