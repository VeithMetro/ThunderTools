...
namespace ProxyStubs {
    ...
    static ProxyStub::MethodHandler ExchangeStreamStubMethods[] = {
        ...
        [](Core::ProxyType<Core::IPCChannel>& channel, Core::ProxyType<RPC::InvokeMessage>& message) {
            ...
            RPC::Data::Frame::Reader reader(message->Parameters().Reader());
            Core::instance_id _parameter_9bfe2652InstanceId__ = reader.Number<Core::instance_id>();

            Exchange::IStream::ICallback* _parameter_9bfe2652{};
            ProxyStub::UnknownProxy* _parameter_9bfe2652Proxy__ = nullptr;
            if (_parameter_9bfe2652InstanceId__ != 0) {
                _parameter_9bfe2652Proxy__ = RPC::Administrator::Instance().ProxyInstance(channel, _parameter_9bfe2652InstanceId__, false, _parameter_9bfe2652);
                ASSERT((_parameter_9bfe2652 != nullptr) && (_parameter_9bfe2652Proxy__ != nullptr));
            }

            implementation->Callback(_parameter_9bfe2652);

            if (_parameter_9bfe2652Proxy__ != nullptr) {
                RPC::Administrator::Instance().Release(_parameter_9bfe2652Proxy__, message->Response());
            }
        },
    ...
    public:
        ...
        void Callback(Exchange::IStream::ICallback* _parameter_9bfe2652) override
        {
            IPCMessage message(static_cast<const ProxyStub::UnknownProxy&>(*this).Message(4));

            RPC::Data::Frame::Writer writer(message->Parameters().Writer());
            writer.Number<Core::instance_id>(RPC::instance_cast(_parameter_9bfe2652));
