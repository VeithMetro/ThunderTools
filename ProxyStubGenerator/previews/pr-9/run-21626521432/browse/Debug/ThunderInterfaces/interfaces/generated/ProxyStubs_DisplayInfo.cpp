...
namespace ProxyStubs {
    ...
    static ProxyStub::MethodHandler ExchangeConnectionPropertiesStubMethods[] = {
        ...
        [](Core::ProxyType<Core::IPCChannel>& channel, Core::ProxyType<RPC::InvokeMessage>& message) {
            ...
            RPC::Data::Frame::Reader reader(message->Parameters().Reader());
            Core::instance_id _parameter_eb2266c6InstanceId__ = reader.Number<Core::instance_id>();

            Exchange::IConnectionProperties::INotification* _parameter_eb2266c6{};
            ProxyStub::UnknownProxy* _parameter_eb2266c6Proxy__ = nullptr;
            if (_parameter_eb2266c6InstanceId__ != 0) {
                _parameter_eb2266c6Proxy__ = RPC::Administrator::Instance().ProxyInstance(channel, _parameter_eb2266c6InstanceId__, false, _parameter_eb2266c6);
                ASSERT((_parameter_eb2266c6 != nullptr) && (_parameter_eb2266c6Proxy__ != nullptr));
            }

            uint32_t result = implementation->Register(_parameter_eb2266c6);

            RPC::Data::Frame::Writer writer(message->Response().Writer());
            writer.Number<uint32_t>(result);

            if (_parameter_eb2266c6Proxy__ != nullptr) {
                RPC::Administrator::Instance().Release(_parameter_eb2266c6Proxy__, message->Response());
            }
        },
        ...
        [](Core::ProxyType<Core::IPCChannel>& channel, Core::ProxyType<RPC::InvokeMessage>& message) {
            ...
            RPC::Data::Frame::Reader reader(message->Parameters().Reader());
            Core::instance_id _parameter_f175c4cdInstanceId__ = reader.Number<Core::instance_id>();

            Exchange::IConnectionProperties::INotification* _parameter_f175c4cd{};
            ProxyStub::UnknownProxy* _parameter_f175c4cdProxy__ = nullptr;
            if (_parameter_f175c4cdInstanceId__ != 0) {
                _parameter_f175c4cdProxy__ = RPC::Administrator::Instance().ProxyInstance(channel, _parameter_f175c4cdInstanceId__, false, _parameter_f175c4cd);
                ASSERT((_parameter_f175c4cd != nullptr) && (_parameter_f175c4cdProxy__ != nullptr));
            }

            uint32_t result = implementation->Unregister(_parameter_f175c4cd);

            RPC::Data::Frame::Writer writer(message->Response().Writer());
            writer.Number<uint32_t>(result);

            if (_parameter_f175c4cdProxy__ != nullptr) {
                RPC::Administrator::Instance().Release(_parameter_f175c4cdProxy__, message->Response());
            }
        },
    ...
    public:
        ...
        uint32_t Register(Exchange::IConnectionProperties::INotification* _parameter_eb2266c6) override
        {
            IPCMessage message(static_cast<const ProxyStub::UnknownProxy&>(*this).Message(0));

            RPC::Data::Frame::Writer writer(message->Parameters().Writer());
            writer.Number<Core::instance_id>(RPC::instance_cast(_parameter_eb2266c6));
        ...
        uint32_t Unregister(Exchange::IConnectionProperties::INotification* _parameter_f175c4cd) override
        {
            IPCMessage message(static_cast<const ProxyStub::UnknownProxy&>(*this).Message(1));

            RPC::Data::Frame::Writer writer(message->Parameters().Writer());
            writer.Number<Core::instance_id>(RPC::instance_cast(_parameter_f175c4cd));
