...
namespace ProxyStubs {
    ...
    static ProxyStub::MethodHandler CompositPluginStubMethods[] = {
        ...
        [](Core::ProxyType<Core::IPCChannel>& channel, Core::ProxyType<RPC::InvokeMessage>& message) {
            ...
            hresult = [&]() -> Core::hresult {
                ...
                RPC::Data::Frame::Reader reader(message->Parameters().Reader());
                if (reader.Length() < (sizeof(Core::instance_id))) { return (COM_ERROR | Core::ERROR_READ_ERROR); }
                Core::instance_id _parameter_702e6575InstanceId__ = reader.Number<Core::instance_id>();

                ICompositPlugin::ICallback* _parameter_702e6575{};
                ProxyStub::UnknownProxy* _parameter_702e6575Proxy__ = nullptr;
                if (_parameter_702e6575InstanceId__ != 0) {
                    _parameter_702e6575Proxy__ = RPC::Administrator::Instance().ProxyInstance(channel, _parameter_702e6575InstanceId__, false, _parameter_702e6575);
                    ASSERT((_parameter_702e6575 != nullptr) && (_parameter_702e6575Proxy__ != nullptr));
                    if ((_parameter_702e6575 == nullptr) || (_parameter_702e6575Proxy__ == nullptr)) { return (COM_ERROR | Core::ERROR_NOT_EXIST); }
                }

                uint32_t result = implementation->Callback(_parameter_702e6575);

                RPC::Data::Frame::Writer writer(message->Response().Writer());
                writer.Number<uint32_t>(result);

                if (_parameter_702e6575Proxy__ != nullptr) {
                    RPC::Administrator::Instance().Release(_parameter_702e6575Proxy__, message->Response());
                }
    ...
    public:
        ...
        uint32_t Callback(ICompositPlugin::ICallback* _parameter_702e6575) override
        {
            IPCMessage message(static_cast<const ProxyStub::UnknownProxy&>(*this).Message(0));

            RPC::Data::Frame::Writer writer(message->Parameters().Writer());
            writer.Number<Core::instance_id>(RPC::instance_cast(_parameter_702e6575));

            const RPC::InstanceRecord passedInstances[] = { { RPC::instance_cast(_parameter_702e6575), ICompositPlugin::ICallback::ID }, { 0, 0 } };
            static_cast<const ProxyStub::UnknownProxy&>(*this).Channel()->CustomData(passedInstances);
